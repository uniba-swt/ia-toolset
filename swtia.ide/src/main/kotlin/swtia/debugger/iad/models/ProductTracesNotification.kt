...
class ProductTracesNotification(val items: List<TextLocationItem>) {
    companion object {
        fun List<MemProductTraceRecord>.toProductTracesNotification(): ProductTracesNotification {
            val items = this.map { re -> re.toTextLocationItem() }
            return ProductTracesNotification(items)
        }
    }
}